...
def save_poem_as_svg(poem_text, filename_base, font_family="Lohit Devanagari"):
    ...
    inkscape_executable = "/home/shree/Downloads/Inkscape-b0a8486-x86_64.AppImage"

    if not os.path.exists(inkscape_executable):
        print(f"\u274C Inkscape executable not found at: {inkscape_executable}")
        print("Please adjust the 'inkscape_executable' variable to your Inkscape installation path.")
        return None
    ...
    line_height = 50
    start_y = 100# Starting y position for the first line 
    svg_height = start_y + len(lines) * line_height + 50 # Add some padding

    svg = f'''<?xml version="1.0" encoding="UTF-8" standalone="no"?>
<svg width="800" height="{svg_height}" xmlns="http://www.w3.org/2000/svg">
  <g font-family="{font_family}" font-size="32" text-anchor="middle" fill="#000">
'''
    ...
    inkscape_command = [
        ...
        "--actions=select-all;object-to-path;export-do" # Select all objects and convert to path, then export
    ]
    try:
        ...
        print(f"\u274C Inkscape path conversion failed (return code {e.returncode}):")
        print(f"Command: {' '.join(e.cmd)}")
        print("Inkscape stdout:\n", e.stdout)
        print("Inkscape stderr:\n", e.stderr)
        print("Ensure the specified font is installed and Inkscape path is correct.")
        return None
    except subprocess.TimeoutExpired:
        print("\u274C Inkscape path conversion timed out after 60 seconds.")
        return None
    except FileNotFoundError:
        print(f"\u274C Inkscape executable not found. Make sure '{inkscape_executable}' is correct.")
        return None
